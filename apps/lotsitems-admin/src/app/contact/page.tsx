        ...
        <div className="min-h-screen relative overflow-hidden">
            ...
            <nav className="container mx-auto px-6 py-6 flex justify-between items-center border-b border-slate-800/50 sticky top-0 z-50 bg-black/40 backdrop-blur-xl">
                ...
                <div className="flex items-center gap-6 text-sm font-medium text-slate-400">
                    ...
                    <Link href="/contact" className="text-white border-b border-blue-500 pb-0.5">{t("navContact")}</Link>
                    <LanguageSelector />
            ...
            <section className="container mx-auto px-6 pt-28 pb-16 text-center max-w-3xl">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold uppercase tracking-wider mb-8">
                    <MessageSquare className="w-3.5 h-3.5" /> {t("navContact")}
                </div>
            ...
            <section className="container mx-auto px-6 py-16 border-t border-slate-800/50">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 max-w-6xl mx-auto">
                    ...
                    <div className="lg:col-span-3">
                        <h2 className="text-3xl font-bold text-white mb-2">{t("contactFormTitle")}</h2>
                        <p className="text-slate-400 mb-8">{t("contactFormSubtitle")}</p>
                        ...
                        {status === "success" ? (
                            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-12 text-center animate-in zoom-in-95 duration-300">
                                ...
                                <h3 className="text-2xl font-bold text-white mb-2">{t("contactSuccessTitle")}</h3>
                                <p className="text-slate-400 mb-6">{t("contactSuccessSubtitle")}</p>
                                <button
                                ...
                                >
                                    {t("contactSuccessBtn") || "Send another message"} <ArrowRight className="w-3.5 h-3.5" />
                                </button>
                            </div>
                        ) : (
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-1.5">{t("contactNameLabel") || "Your Name"}</label>
                                        <input
                                            type="text"
                                            required
                                            value={form.name}
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-1.5">{t("contactEmailLabel") || "Email Address"}</label>
                                        <input
                                            type="email"
                                            required
                                            value={form.email}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1.5">{t("contactSubjectLabel") || "Subject"}</label>
                                    <select
                                        required
                                        value={form.subject}
                                    ...
                                    >
                                        <option value="" disabled>{t("contactSubjectPlaceholder") || "Select a topic..."}</option>
                                        <option value="selling">{t("contactSubjectSell") || "I want to sell a device"}</option>
                                        <option value="vendor">{t("contactSubjectVendor") || "Become a verified vendor"}</option>
                                        <option value="order">{t("contactSubjectOrder") || "Question about my order"}</option>
                                        <option value="partnership">{t("contactSubjectPartnership") || "Partnership / B2B inquiry"}</option>
                                        <option value="other">{t("contactSubjectOther") || "Other"}</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1.5">{t("contactMessageLabel") || "Message"}</label>
                                    <textarea
                                        required
                                        rows={6}
                                        value={form.message}
                                        ...
                                        placeholder={t("contactMessagePlaceholder") || "Tell us how we can help..."}
                                    />
                                </div>
                                <button
                                    type="submit"
                                ...
                                >
                                    {status === "loading" ? (
                                        <><Loader2 className="w-5 h-5 animate-spin" /> {t("contactSendingBtn") || "Sending..."}</>
                                    ) : (
                                        <>{t("contactSendBtn") || "Send Message"} <ArrowRight className="w-4 h-4" /></>
                                    )}
                                </button>
                            </form>
                        )}
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                            <h3 className="font-bold text-white mb-4 text-lg">{t("contactFaqTitle") || "Frequently Asked"}</h3>
                            <div className="space-y-5">
                                {[
                                    { q: t("faq1Q") || "How long does a payout take?", a: t("faq1A") || "Most payouts complete within 24 hours of our engineer verifying your device." },
                                    { q: t("faq2Q") || "How do I become a vendor?", a: t("faq2A") || "Fill in the contact form and select 'Become a verified vendor'. Our team will reach out within 24 hours." },
                                    { q: t("faq3Q") || "Is there a listing fee?", a: t("faq3A") || "No — listing your device is completely free. Vendors pay a small commission on successful bids." },
                                    { q: t("faq4Q") || "What devices do you accept?", a: t("faq4A") || "Smartphones, tablets, laptops, and smartwatches from all major brands." },
                                ].map((faq, i) => (
                        ...
                        <div className="bg-gradient-to-br from-blue-600/20 to-purple-600/10 border border-blue-500/20 rounded-2xl p-6">
                            <h3 className="font-bold text-white mb-2">{t("profileUpgradeSoon")}</h3>
                            <p className="text-slate-400 text-sm mb-4">{t("profileUpgradeDesc")}</p>
                            <Link href="/offer" className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 text-sm">
                                {t("navGetOffer")} <ArrowRight className="w-4 h-4" />
                            </Link>
                        </div>
                    </div>
                </div>
