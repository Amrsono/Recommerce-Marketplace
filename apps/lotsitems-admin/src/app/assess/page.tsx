...
export default function AssessPage() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                const res = await fetch(`${'/api'}/devices/handoff/${handoffSessionId}`);
                if (!res.ok) return;
    ...
    const submitOffer = async (finalDevice: DeviceInfo, estimatedPrice: number) => {
        ...
        try {
            const res = await fetch(`${'/api'}/devices/submit`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
...
function FinalOfferCard({
    ...
    const handleRejectEstimate = async () => {
        ...
        try {
            ...
            await fetch(`${'/api'}/profile/setup-test-user`, { method: 'POST' });

            // Create the device and ticket directly with REJECTED status
            await fetch(`${'/api'}/devices/submit`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
